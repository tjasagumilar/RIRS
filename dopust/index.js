    ...
    const enrichedDopusti = await Promise.all(
      dopusti.map(async (dopust) => {
        try {
          ...
          const employeeData = employeeResponse.data[0]; // Assuming response is an array
          return {
...
app.post('/dopusti', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: savedDopust._id.toString() });
  } catch (err) {
    res.status(500).json({ error: "Error saving dopust" });
  }
});

app.post('/kolektivni', async (req, res) => {
  const { employeeIds, zacetek, konec, opis, status } = req.body;

  // Validate the required fields
  if (!employeeIds || !Array.isArray(employeeIds) || !zacetek || !konec || !opis || !status) {
    return res.status(400).json({ error: "Missing fields or invalid data" });
  }

  try {
    // Create an array to hold the saved leave entries
    const savedDopusts = [];

    // Loop through each employee ID and create a leave entry
    ...
    // Respond with the IDs of the saved leave entries
    res.status(201).json({
...
app.delete('/zavrnjeni', async (req, res) => {
  try {
    ...
    console.error("Error deleting records:", err);  // Log the error
